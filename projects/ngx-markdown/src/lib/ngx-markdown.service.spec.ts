...
import { MarkdownService, errorKatexNotLoaded } from './ngx-markdown.service';
import { MarkedOptions } from '../public-api';
...
describe('MarkdowService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      ...
      providers: [{ provide: MarkedOptions, useValue: {} }, MarkdownService]
    });
  });
  ...
  describe('compile', () => {
    ...
    it('should sanitize when markedOptions.sanitize is true and no sanitizer function is provided', () => {
      ...
      expect(markdownService.compile(mockRaw, false, markedOptions)).toBe(
        sanitized
      );
      expect(markdownService.compile(mockRaw, false, markedOptions)).not.toBe(
        unsanitized
      );
    });
    ...
    it('should not sanitize when markedOptions.sanitize is false regardless of whether a sanitizer function is provided or not', () => {
      ...
      expect(markdownService.compile(mockRaw, false, { sanitize: false })).toBe(
        expected
      );
      expect(
  ...
  describe('getSource', () => {
    it('should call http service to get src content', () => {
      ...
      markdownService.getSource(mockSrc).subscribe(data => (result = data));
  ...
  describe('renderKatex', () => {
    it('should throw when katex is called but not loaded', () => {
      ...
      expect(() => markdownService.renderKatex('$example$')).toThrowError(
        errorKatexNotLoaded
      );
    ...
    it('should call katex with math expressions', () => {
      global['katex'] = {
        renderToString: (tex: string, options?: KatexOptions) => ''
      };
